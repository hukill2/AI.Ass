...
function fail(message) {
  console.error(`Eligible candidate review status non-pending error: ${message}`);
  process.exit(1);
}
...
if (eligible.length !== 1) {
  console.log(
    `Eligible candidate review status non-pending check skipped (${eligible.length} eligible candidate${
      eligible.length === 1 ? '' : 's'
    } present).`
  );
  process.exit(0);
...
if (!status) {
  fail(`review ${reviewId} for execution_id=${execId} missing operator_status`);
}

if (status === 'pending') {
  fail(`review ${reviewId} for execution_id=${execId} is still pending`);
}
