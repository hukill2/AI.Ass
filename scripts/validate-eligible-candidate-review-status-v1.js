...
const REJECTING_STATUSES = new Set(['rejected', 'denied']);

function fail(message) {
  console.error(`Eligible candidate review status error: ${message}`);
  process.exit(1);
}
...
if (eligible.length !== 1) {
  console.log(
    `Eligible candidate review-status check skipped (${eligible.length} eligible candidate${
      eligible.length === 1 ? '' : 's'
    } present).`
  );
  process.exit(0);
...
if (!review) {
  fail(`review ${reviewId} not found for execution_id=${execId}`);
}

const status = (review.operator_status || '').toLowerCase();
if (REJECTING_STATUSES.has(status)) {
  fail(
    `review ${reviewId} for execution_id=${execId} has rejecting status "${review.operator_status}"`
  );
}
