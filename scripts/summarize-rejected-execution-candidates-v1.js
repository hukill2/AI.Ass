#!/usr/bin/env node
// Usage: node scripts/summarize-rejected-execution-candidates-v1.js
...
if (rejectedCandidates.length === 0) {
  console.log('No execution candidates are currently rejected.');
  process.exit(0);
}

const lines = rejectedCandidates.map((candidate) => {
  const review = reviewsDoc.reviews.find((r) => r.review_id === candidate.review_id);
...
console.log('Rejected execution candidates:');
console.log('execution_id | task_id | review_status | recommended_next_step | files_to_create_or_update');
lines.forEach((line) => console.log(line));
console.log(`Total rejected candidates: ${rejectedCandidates.length}`);
