#!/usr/bin/env node
// Usage: node scripts/validate-rejected-execution-state-v1.js
...
const rejectedCandidates = candidatesDoc.candidates.filter((candidate) => {
...
if (rejectedCandidates.length === 0) {
  console.log('No rejected execution candidates to validate.');
  process.exit(0);
}
...
for (const candidate of rejectedCandidates) {
  ...
  if (review.operator_status !== 'rejected') {
    issues.push(`execution_id ${candidate.execution_id} review ${review.review_id} is not marked rejected`);
    continue;
  }
...
if (issues.length > 0) {
  console.error('Rejected state validator found issues:');
  issues.forEach((issue) => console.error(`- ${issue}`));
  process.exit(1);
}

console.log(`Rejected execution state is consistent (${rejectedCandidates.length} candidate${rejectedCandidates.length === 1 ? '' : 's'}).`);
process.exit(0);
