#!/usr/bin/env node
// Usage: node scripts/summarize-execution-candidate-ops-status-v1.js
...
try {
  ...
  console.error(`Failed to produce tooling manifest: ${err.message}`);
  process.exit(1);
}
...
console.log(`  coverage -> approved:${buckets.approved} pending:${buckets.pending} rejected:${buckets.rejected} unreviewed:${buckets.unreviewed} anomalies:${buckets.anomalies} total:${totalCandidates}`);
console.log(`  tooling  -> present:${present} missing:${missing} (from manifest)`);
process.exit(0);
