...
function runStage(stageName) {
  ...
  if (stageName === 'preflight') {
    ...
    const payloadResult = runScript('validate-eligible-candidate-payloads-v1');
    ...
    const readonlyDecisionResult = runScript('validate-eligible-candidate-readonly-log-decision-match-v1');
    if (readonlyDecisionResult.status !== 0) {
      logStageFailure(stageName, 'validate-eligible-candidate-readonly-log-decision-match-v1');
      return {
        stage: stageName,
        status: 'failed',
        script: 'validate-eligible-candidate-readonly-log-decision-match-v1',
        detail: null
      };
    }
    const readonlyExecutionResult = runScript('validate-eligible-candidate-readonly-log-execution-id-match-v1');
    if (readonlyExecutionResult.status !== 0) {
      logStageFailure(stageName, 'validate-eligible-candidate-readonly-log-execution-id-match-v1');
      return {
        stage: stageName,
        status: 'failed',
    ...
    const reviewResult = runScript('validate-eligible-candidate-review-v1');
    ...
    if (taskResult.status !== 0) {
      ...
      return { stage: stageName, status: 'failed', script: 'validate-eligible-candidate-task-context-v1', detail: null };
    }
    const decisionResult = runScript('validate-eligible-candidate-decision-linkage-v1');
    if (decisionResult.status !== 0) {
    ...
    if (writeResult.status !== 0) {
      ...
      return { stage: stageName, status: 'failed', script: 'validate-eligible-candidate-no-prior-write-v1', detail: null };
    }
