// Usage: node scripts/list-approval-candidates-v1.js
const fs = require('fs');
...
function main() {
  ...
  const candidates = reviews.filter(
    (review) =>
      review.classification === 'approval-required' && review.operator_status === 'reviewed'
  );
  console.log(`Total approval candidates: ${candidates.length}`);
  if (!candidates.length) {
    console.log('No approval-required reviews are awaiting explicit approval.');
    return;
  }
