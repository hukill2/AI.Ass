#!/usr/bin/env node
// Usage: node scripts/validate-approved-awaiting-execution-state-v1.js
...
if (awaitingApproved.length === 0) {
  console.log('No approved candidates awaiting execution.');
  process.exit(0);
}

const issues = [];

for (const candidate of awaitingApproved) {
  ...
  if (review.operator_status !== 'approved') {
    issues.push(`execution_id ${candidate.execution_id} review ${review.review_id} is not operator approved`);
    continue;
  }
  ...
  if (!decision.recommended_next_step || typeof decision.recommended_next_step !== 'string' || !decision.recommended_next_step.trim()) {
    issues.push(`execution_id ${candidate.execution_id} decision ${decision.decision_id} lacks recommended_next_step`);
  }
}

if (issues.length > 0) {
  console.error('Approved-awaiting-execution state validator found issues:');
  issues.forEach((issue) => console.error(`- ${issue}`));
  process.exit(1);
}

console.log(`Approved-awaiting-execution state is consistent (${awaitingApproved.length} candidate${awaitingApproved.length === 1 ? '' : 's'}).`);
process.exit(0);
