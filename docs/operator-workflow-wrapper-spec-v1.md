...
1. **Preflight stage**: before touching the prompt-template guard, run `validate-execution-candidates-schema-v1.js` so the wrapper ensures the candidate ledger exposes the required IDs, statuses, and context the auto-selection logic just relies on. Then, if `--execution-id=<id>` was provided, run `validate-local-write-readiness-v1.js --execution-id=<id>` to confirm the write-lane prerequisites for that execution. Next run `check-prompt-template-mirror-v1.js`, followed by `validate-execution-candidate-anomalies-v1.js`, `validate-all-review-lanes-state-v1.js`, `validate-execution-candidate-coverage-buckets-v1.js`, `validate-execution-candidate-view-coverage-v1.js`, and `validate-unreviewed-execution-state-v1.js`. When the flag is omitted, the wrapper auto-selects a single awaiting/prepared execution candidate (if one exists) for the readiness check; if zero or multiple eligible candidates are present it logs the skip reason and proceeds with the same script order.
