...
1. **Preflight stage**: if `--execution-id=<id>` was provided, run `validate-local-write-readiness-v1.js --execution-id=<id>` before the remaining scripts so the wrapper first confirms the write-lane prerequisites for that execution. Next run `check-prompt-template-mirror-v1.js` to guard the prompt-template mirror, followed by `validate-execution-candidate-anomalies-v1.js`, `validate-all-review-lanes-state-v1.js`, `validate-execution-candidate-coverage-buckets-v1.js`, `validate-execution-candidate-view-coverage-v1.js`, and `validate-unreviewed-execution-state-v1.js`. When the flag is omitted, the wrapper prints `Skipping local write readiness check: no --execution-id provided.` but otherwise follows the same script order.
