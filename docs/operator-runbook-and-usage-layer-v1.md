...
- **Executor command examples:**
  * **Readonly:** `node scripts/execute-codex-readonly-check-v1.js --execution-id ...` then `node scripts/execute-codex-readonly-v1.js --execution-id ...`; consult `local-readonly-executor-status-v1.md` for the required execution_id/preview inputs.
  ...
  * **Codex:** run `node scripts/prepare-codex-handoff-dry-run-v1.js --payload-id ...` and `node scripts/prepare-codex-invocation-preview-v1.js --handoff-id ...` before calling `node scripts/execute-codex-approved-item-dryrun-v1.js --execution-id ...`; see `codex-execution-contract-v1.md` for the payload/hand-off/previews you must stage.
...
- **Codex readiness reminder:** before treating the Codex lane as ready, ensure the Codex handoff packet and invocation preview artifacts referenced in `docs/codex-handoff-dry-run-contract-v1.md` and `docs/codex-invocation-preview-contract-v1.md` are prepared per `docs/codex-execution-contract-v1.md`; those files describe the exact payloads the wrapper should see before any codex command runs.
- **Quick-reference tip:** Whenever you need a fast lookup for those readiness artifacts, preconditions, result values, and guardrails, open `docs/codex-contract-quick-reference-v1.md` before diving deeper into the full contracts.
